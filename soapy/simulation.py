...
class Sim(object):
    ...
    def runWfs_noMP(self, scrns = None, dmShape=None, wfsList=None,
        ...
        for nwfs in wfsList:
            ...
            if loopIter: # loopIter = self.iters
                read=False
                if (int(float(self.config.sim.loopTime*loopIter)
                    ...
                    read=True
                # if exposureTime = loopTime, the WFS is always read. 
                # if, for example, 
            else:
                read = True
            ...
            # if read is false, self.slopes won't be updated
    ...
    def loopFrame(self):
        ...
        # if a DM is in closed-loop, then its corrsponding open_correction is all 0 and close_correction is not.
...
#                    self.sciPhase[sci][i] = self.sciCams[sci].residual
                    self.sciPhase[sci][i] = self.sciCams[sci].los.residual # added by Xuewen
