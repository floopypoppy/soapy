...
class Reconstructor(object):
    ...
    def __init__(self, soapy_config, dms, wfss, atmos, runWfsFunc=None):
        ...
        for dm in xrange(self.sim_config.nDM):
            self.dmActs.append(self.dms[dm].dmConfig.nxActuators) # number of actuators in one dimension
        ...
        n_acts = 0
        self.first_acts = [] # [0, 64, ...]
        for i, dm in self.dms.items():
            self.first_acts.append(n_acts)
            n_acts += dm.n_acts # number of all actuators in each dm

        n_wfs_measurements = 0
        self.first_measurements = [] # [0, 72, ...]
        for i, wfs in self.wfss.items():
            self.first_measurements.append(n_wfs_measurements)
            n_wfs_measurements += wfs.n_measurements # number of measurements of each WFS
    ...
    def find_closed_actuators(self):
        """
        it's an array of shape (totalActs,) and is 1 at closed DM actuators and 0 elsewhere.
        """
    ...
    def apply_gain(self):
        ...
        for dm_i, dm in self.dms.items():
            ...
            dm.n_valid_actuators = dm.getActiveActs() # added by Xuewen
